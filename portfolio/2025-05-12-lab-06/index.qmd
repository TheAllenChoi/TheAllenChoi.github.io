---
title: "Lab 6: Code Efficiency - Spicy"
...
remove_outliers <- function(df, ..., sd_thresh = 3) {
  ...
  ## check that all vars are numeric
    ### thank you Dr. Theobold for the code example
  numeric_total <- df %>%
  ...
  if (length(numeric_total) != length(var_names)) {
    stop("All of the inputted variables must be numeric")
  }
  
  bool <- df |> 
    select(var_names) |> 
    mutate(across(var_names, list(mean = mean, sd = sd))) |> 
    
    # thank you to https://stackoverflow.com/a/67031255
    reframe(across(var_names, ~(.x - mean(.x)) / sd(.x))) |> 
    
    reframe(across(var_names, ~(.x > -sd_thresh & .x < sd_thresh))) |> 
    rowSums()
  return(df[(bool == length(var_names)), ])
}
```
...
impute_missing <- function(df, ..., impute_fun = mean) {
  fun <- enquos(impute_fun, .named = TRUE)
  vars <- enquos(..., .named = TRUE)
  var_names <- names(vars)
  ...
  if (names(fun) == "mean") {
    df <- df |>
      mutate(across(var_names, ~(replace_na(.x, mean(.x)))))
    
  } else if (names(fun) == "median") {
    df <- df |>
      mutate(across(var_names, ~(replace_na(.x, median(.x)))))
    
  } else {
    stop("Impute Function must be 'mean' or 'median'")
  }
  return(df)
}
```
...
```{r}
#| label: exercise-3
fit_model <- function(df, mod_formula, remove_outliers = FALSE,
                      impute_missing = FALSE, ...) {
  x_var <- enquos(...)[2]

  if (remove_outliers) {
    df <- remove_outliers(df, !!!x_var)
  }
  if (impute_missing) {
    df <- impute_missing(df, !!!x_var)
  } 
  
  formula <- enquos(mod_formula, .named = TRUE)

  return(lm(names(formula), df))
}
